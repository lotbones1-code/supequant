...
class TradingChecklistFilter:
    ...
    def _score_sentiment(self, market_state: Dict) -> float:
        ...
        # ==========================================
        # REAL Fear & Greed Index (Elite Integration)
        # ==========================================
        try:
            from data_feed.sentiment_tracker import get_sentiment_tracker
            ...
            if fng and fng.get('source') != 'fallback':
                ...
                # Map Fear & Greed to sentiment score
                # Extreme Fear (0-24): Contrarian bullish = score boost
                # Fear (25-44): Cautiously bullish
                # Neutral (45-55): No adjustment
                # Greed (56-74): Cautiously bearish
                # Extreme Greed (75-100): Contrarian bearish
                
                if fng_value <= 24:
                    score += 20  # Extreme fear = buying opportunity
                elif fng_value <= 44:
                    score += 10  # Fear = favorable
                elif fng_value <= 55:
                    pass  # Neutral
                elif fng_value <= 74:
                    score -= 5  # Greed = cautious
                else:
                    score -= 15  # Extreme greed = caution
                    
                # Store for logging
                self._last_fng = fng_value
                self._last_fng_class = classification
        except Exception as e:
            # Fallback if sentiment tracker unavailable
            pass
