...
class DebugAgent:
    ...
    def _debug_no_signals(self, strategy: str, backtest_results: Dict) -> Dict:
        ...
        # Try multiple strategy file names (V1, V2, V3)
        strategy_files = [
            self.project_root / "strategy" / f"{strategy}_strategy_v3.py",
            self.project_root / "strategy" / f"{strategy}_strategy_v2.py",
            self.project_root / "strategy" / f"{strategy}_strategy.py",
        ]
        ...
        if not strategy_file:
            return {'fix_available': False, 'error': f'Strategy file not found. Tried: {[str(sf) for sf in strategy_files]}'}
        
        logger.info(f"📖 Reading strategy file: {strategy_file.name}")
        with open(strategy_file, 'r') as f:
            strategy_code = f.read()
        
        # Build comprehensive context
        # Read backtest engine to understand data flow
        backtest_engine_file = self.project_root / "backtesting" / "backtest_engine.py"
        backtest_engine_code = ""
        if backtest_engine_file.exists():
            with open(backtest_engine_file, 'r') as f:
                backtest_engine_code = f.read()[:2000]  # First 2000 chars
        
        # Read data loader to check data loading
        data_loader_file = self.project_root / "backtesting" / "historical_data_loader.py"
        data_loader_code = ""
        if data_loader_file.exists():
            with open(data_loader_file, 'r') as f:
                data_loader_code = f.read()[:2000]  # First 2000 chars
...
Strategy File: {strategy_file.name}
Strategy Code (first 4000 chars):
```python
{strategy_code[:4000]}
```

Backtest Engine (relevant parts):
```python
{backtest_engine_code}
```

Data Loader (relevant parts):
```python
{data_loader_code}
```
"""
        
        system_prompt = """You are an expert debugger for trading strategies and backtesting systems.

Analyze why signals aren't generating and provide specific code fixes.
Check:
1. Strategy conditions (thresholds, filters)
2. Data loading issues (zero candles, wrong date ranges)
3. Market state building problems
4. Indicator calculation issues

Focus on:
- Threshold values that are too strict
- Data loading problems (check if candles are actually loaded)
- Logic errors in signal generation
- Missing or incorrect indicator calculations"""
        
        prompt = f"""The {strategy} strategy (file: {strategy_file.name}) is generating ZERO signals in backtest.

{context}

CRITICAL: First check if data is actually loaded. Look at:
- Are candles being loaded? (check backtest results summary)
- Is the data loader using the correct API endpoint?
- Are the date ranges correct?

Then analyze the strategy code and provide:
1. Data loading issues (if any)
2. Which conditions are blocking signals
3. Current threshold values
4. Recommended threshold adjustments
5. Specific code changes (with line numbers if possible)
6. Expected impact

Format your response as JSON:
{{
    "data_issues": ["issue1", "issue2"] or [],
    ...
    "recommended_changes": [
        {{
            ...
            "line": 80,
            "old_code": "if volume_ratio < 2.5:",
            "new_code": "if volume_ratio < 2.0:",
            "reasoning": "Lower threshold to allow more signals while staying realistic"
        }}
    ],
...
def main():
    ...
    parser.add_argument('--strategy', type=str, default='breakout', 
                       help='Strategy to debug (breakout, pullback, etc. - will auto-detect V3/V2/V1)')
