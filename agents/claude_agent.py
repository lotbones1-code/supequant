...
class ClaudeAgent:
    ...
    def __init__(self, api_key: Optional[str] = None, model: str = "claude-sonnet-4-20250514", 
        ...
        logger.info(f"✅ ClaudeAgent initialized (model: {self.model}, timeout: {timeout_seconds}s)")
    ...
    def _call_claude(self, messages: List[Dict], system: str = "", max_tokens: int = 4096, 
                    fail_open: bool = True) -> Dict:
        """
        Call Claude API with retry logic, circuit breaker, and timeout protection
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt < self.max_retries - 1:
                    ...
                    if fail_open:
                        logger.warning("⚠️  Claude API failed - returning safe default (fail-open)")
                        return {
                            'content': f"Claude API unavailable: {str(e)[:100]}",
                            'usage': {'input_tokens': 0, 'output_tokens': 0},
                            'error': str(e),
                            'circuit_breaker_blocked': False
                        }
        ...
        if fail_open:
            return {
                'content': "Claude API request failed after all retries",
                'usage': {'input_tokens': 0, 'output_tokens': 0},
                'circuit_breaker_blocked': False
            }
...
5. Implementation steps"""
    ...
    def get_health_status(self) -> Dict:
        """Get health status of Claude agent"""
        return {
            'available': ANTHROPIC_AVAILABLE and self.api_key is not None,
            'circuit_breaker_state': self.circuit_breaker_state.value,
            'circuit_breaker_failures': self.circuit_breaker_failures,
